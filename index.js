...
export function apply(ctx, config) {
  ...
  const api = async (req, res) => {
    try {
      ...
      if (path === '/synapse/api/export' && req.method === 'POST') {
        if (exportDir === null) return sendJson(res, 400, { error: '未配置导出目录' })
        const body = await readJson(req, 8 * 1024 * 1024)
        const filename = typeof body?.filename === 'string' ? body.filename : ''
        const content = typeof body?.content === 'string' ? body.content : ''
        // Only a plain *.canvas file name may be written; no path separators.
        if (!/^[^\\/:*?"<>|]+\.canvas$/.test(filename)) return sendJson(res, 400, { error: '文件名不合法' })
        if (content.length > 8 * 1024 * 1024) return sendJson(res, 400, { error: '内容过大' })
        try { JSON.parse(content) } catch { return sendJson(res, 400, { error: 'canvas 内容不是有效 JSON' }) }
        await mkdir(exportDir, { recursive: true })
        const target = join(exportDir, filename)
        await writeFile(target, content, 'utf8')
        ctx.logger.info(`synapse: exported canvas -> ${target}`)
