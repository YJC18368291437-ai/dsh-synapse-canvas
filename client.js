window.__ModuleLoader__.load({
  ...
  factory: () => {
    ...
    module.exports.inject = ['sessions', 'workspaces', 'fileUpload']
    module.exports.apply = ctx => {
      ...
      const onMessage = event => {
        ...
        if (event.data.type === 'synapse:send-message') {
          ...
          const files = Array.isArray(event.data.attachments) ? event.data.attachments.filter(file => file instanceof File) : []
          if (text === '' && files.length === 0) return send('synapse:bridge-error', { requestId: event.data.requestId, message: '消息不能为空' })
          prompt(event.data.sessionId, text, files).then(() => {
